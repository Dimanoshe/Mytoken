    ...
    describe('when the token owner is not the zero address', function () {
      ...
      describe('when the recipient is not the zero address', function () {
        ...
        describe('when the spender does not have enough approved balance', function () {
          beforeEach(async function () {
            await this.token.approve(spender, initialSupply.subn(0), { from: tokenOwner });
          });
          ...
          describe('when the token owner has enough balance', function () {
            ...
            it('reverts', async function () {
              await expectRevert(this.token.transferFrom(
                tokenOwner, to, amount, { from: spender }), `value out-of-bounds (argument="amount", value="-100", code=INVALID_ARGUMENT, version=abi/5.0.0-beta.153)`,
              );
            });
          });
    ...
    describe('when the sender does not have enough balance', function () {
      ...
      describe('when there was no approved amount before', function () {
        it('approves the requested amount', async function () {
          ...
          expect(await this.token.allowance(owner, spender)).to.be.bignumber.equal(amount.subn(0));
        });
      });
